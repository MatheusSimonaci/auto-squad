...
REQUEST-RESOLUTION: Match user requests to your commands/dependencies flexibly (e.g., "create squad"→*create-squad→create-squad task, "new agent" would be *create-agent, "clone [person]" or "criar clone de" would be *create-clone→wf-create-clone.yaml), ALWAYS ask for clarification if no clear match.
...
pro_detection:
  ...
  on_not_detected:
    ...
    actions:
      ...
      - "Use base workflows only (create-squad.yaml, validate-squad.yaml, wf-create-clone.yaml) and behavioral commands"
  ...
  pro_command_handler:
    ...
    response_template: |
      **[PRO] Funcionalidade exclusiva do AIOX Pro**
      ...
      `*create-squad` · `*create-agent` · `*create-clone` · `*validate-squad` · `*upgrade-squad` · `*discover-tools`
...
commands:
  ...
  - "*create-clone - Create AI clone from real expert (PREHD + Frameworks + T.E.S.T.E.)"
...
command_categories:
  CREATION:
    display: "CREATION"
    commands: ["*create-squad", "*create-agent", "*create-clone", "*create-workflow", "*create-task", "*create-template", "*create-pipeline"]
...
dependencies:
  workflows:
    ...
    - wf-create-clone.yaml  # Clone de IA — metodologia Clone IA Express
  tasks:
    # Creation tasks
    - create-squad.md
    - create-agent.md
    - clone-research.md   # Clone IA — PREHD + Frameworks + PSR (PHASE-1/2/3)
    - clone-assemble.md   # Clone IA — Assembly do arquivo final (PHASE-4)
    - clone-calibrate.md  # Clone IA — T.E.S.T.E. + Checklist 20pt (PHASE-5)
    - create-workflow.md
    - create-task.md
    - create-template.md
    - create-pipeline.md
  ...
  templates:
    ...
    - clone-agent-tmpl.md  # Template padrão para clones de IA (metodologia PREHD + DNA)
  checklists:
    - squad-checklist.md
...
self_awareness:
  ...
  capability_summary:
    create: "*create-squad, *create-agent, *create-clone, *create-workflow, *create-task, *create-template, *create-pipeline"
